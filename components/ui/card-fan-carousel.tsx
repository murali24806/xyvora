...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  useEffect(() => {
    ...
    let interval = setInterval(() => cycle("right"), 2000);
    ...
    if (container) {
      ...
      const handleMouseLeave = () => {
        ...
        interval = setInterval(() => cycle("right"), 2000);
      };
  ...
  const chevron = (direction: "left" | "right") => {
    return direction === "left" ? (
      <ChevronLeft className="relative z-[2] w-4 h-4 md:w-5 md:h-5" strokeWidth={2.5} />
    ) : (
      <ChevronRight className="relative z-[2] w-4 h-4 md:w-5 md:h-5" strokeWidth={2.5} />
    );
  };
  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20 overflow-hidden">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-[400px] sm:h-[500px] md:h-[600px]">
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-full h-full overflow-hidden rounded-2xl shadow-2xl bg-gray-200 dark:bg-gray-800 border border-white/10">
                <img src={card.imgUrl} loading="lazy" alt={card.alt || `Card ${index}`} className="absolute inset-0 w-full h-full object-cover z-10" />
              </div>
            );
            const cardClasses = "fan-card absolute w-[240px] h-[340px] sm:w-[280px] sm:h-[400px] md:w-[320px] md:h-[460px] origin-center";
            return card.linkUrl ? (
